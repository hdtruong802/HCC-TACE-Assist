...
from src.data.lld_ingest import (  # noqa: E402
    ...
    patient_category, representative_box,
)
...
def main() -> None:
    ...
    for pi, pid in enumerate(patients):
        ...
        for ph in phases:
            ...
            if entry is None or box is None or img is None:
                miss += 1
                continue
            try:
            ...
            rows.append(dict(
                ...
                malignant=int(cat in malignant), phase=ph, slice_idx=box[0],
                bbox_area=round(float(box[3] - box[1]) * float(box[4] - box[2]), 1),
                row=len(images), image_file=paths["image_file"],
            ))
            images.append(roi); kept += 1
        if (pi + 1) % 50 == 0 or pi == len(patients) - 1:
            print(f"[{pi + 1}/{len(patients)}] tổng ROI={len(images)} (miss={miss})")
